...
template <typename T> struct Pass {
  ...
  Pass(const VideoInfo &vi, const VideoInfo &vi2) : vi(vi), vi2(vi2) {}
...
template <typename T> struct TransposePass final : public BridgePass<T> {
  ...
  void process(int, int plane, cudaStream_t stream) override {
    auto sw = !!plane * this->vi.GetPlaneWidthSubsampling(getPlaneId(this->vi, plane));
    auto sh = !!plane * this->vi.GetPlaneHeightSubsampling(getPlaneId(this->vi, plane));
    ...
    transpose<<<grids, blocks, 0, stream>>>(this->src, this->dst, width, height, this->d_pitch_src / sizeof(T) >> sw,
                                            this->d_pitch_dst / sizeof(T) >> sh);
  }
};
...
template <typename T> struct ScaleDownWPass final : public BridgePass<T> {
  ...
  void process(int, int plane, cudaStream_t stream) override {
    ...
    resample12<<<grids, blocks, 0, stream>>>(this->src, this->dst, width, height, this->d_pitch_src >> sw,
                                             this->d_pitch_dst >> sw);
  }
};
...
template <typename T> struct ShiftWPass final : public BridgePass<T> {
  ...
  void process(int, int plane, cudaStream_t stream) override {
    ...
    dim3 blocks = dim3(64, 8);
    dim3 grids = dim3((width - 1) / blocks.x + 1, (height - 1) / blocks.y + 1);
    resample6<<<grids, blocks, 0, stream>>>(this->src, this->dst, width, height, this->d_pitch_src >> sw,
                                            this->d_pitch_dst >> sw);
  }
};
...
public:
  EEDI2Pass(const EEDI2Pass &other) : Pass<T>(other), d(other.d), map(other.map), pp(other.pp), fieldS(other.fieldS) {
    initCuda();
  }

  EEDI2Pass(const VideoInfo &vi, const VideoInfo &vi2, EEDI2Param d, unsigned map, unsigned pp, unsigned fieldS)
...
public:
  ...
  void process(int n, int plane, cudaStream_t stream) override {
    ...
    auto subSampling = plane ? this->vi.GetPlaneWidthSubsampling(getPlaneId(this->vi, plane)) : 0u;
...
public:
  Pipeline(std::string_view filterName, AVSValue args, IScriptEnvironment *env) : node(args[0].AsClip()) {
    using invalid_arg = std::invalid_argument;

    vi = node->GetVideoInfo();
    auto vi2 = vi;
    EEDI2Param d;
    ...
    auto propGetIntDefault = [](const AVSValue &args, int64_t def) { return args.AsInt(def); };

    if (filterName == "EEDI2")
      numeric_cast_to(fieldS, args[1].AsInt());
    else
      fieldS = 1;

    auto num_args = args.ArraySize();
    numeric_cast_to(d.mthresh, propGetIntDefault(args[num_args - 11], 10));
    numeric_cast_to(d.lthresh, propGetIntDefault(args[num_args - 10], 20));
    numeric_cast_to(d.vthresh, propGetIntDefault(args[num_args - 9], 20));

    numeric_cast_to(d.estr, propGetIntDefault(args[num_args - 8], 2));
    numeric_cast_to(d.dstr, propGetIntDefault(args[num_args - 7], 4));
    numeric_cast_to(d.maxd, propGetIntDefault(args[num_args - 6], 24));

    numeric_cast_to(map, propGetIntDefault(args[num_args - 5], 0));
    numeric_cast_to(pp, propGetIntDefault(args[num_args - 3], 1));

    unsigned nt;
    numeric_cast_to(nt, propGetIntDefault(args[num_args - 4], 50));

    numeric_cast_to(device_id, propGetIntDefault(args[num_args - 1], -1));
  ...
  Pipeline(const Pipeline &other) : node(other.node), vi(other.vi), device_id(other.device_id) {
  ...
  const VideoInfo &getOutputVI() const { return passes.back()->getOutputVI(); }
  ...
  PVideoFrame getFrame(int n, IScriptEnvironment *env) {
    ...
    for (int plane = 0; plane < vi.NumComponents(); ++plane) {
      ...
      auto d_pitch_src = passes.front()->getSrcPitch() >> !!plane * vi.GetPlaneWidthSubsampling(getPlaneId(vi, plane));
      auto d_pitch_dst = passes.back()->getDstPitch() >> !!plane * vi2.GetPlaneWidthSubsampling(getPlaneId(vi2, plane));

      // upload
      env->BitBlt(reinterpret_cast<BYTE *>(h_src), d_pitch_src, s_src, s_pitch_src, src_width_bytes, src_height);
      try_cuda(cudaMemcpy2DAsync(d_src, d_pitch_src, h_src, d_pitch_src, src_width_bytes, src_height,
      ...
      for (unsigned i = 0; i < passes.size(); ++i) {
        ...
        if (i) {
          ...
          auto sw = !!plane * last_vi.GetPlaneWidthSubsampling(getPlaneId(vi, plane));
          auto sh = !!plane * last_vi.GetPlaneHeightSubsampling(getPlaneId(vi, plane));
          if (!cur.getSrcDevPtr()) {
          ...
          if (curPtr != lastPtr)
            try_cuda(cudaMemcpy2DAsync(curPtr, cur.getSrcPitch() >> sw, lastPtr, last.getDstPitch() >> sw,
                                       last_vi.width * sizeof(T) >> sw, last_vi.height >> sh, cudaMemcpyDeviceToDevice,
                                       stream));
        }
      ...
      // download
      try_cuda(cudaMemcpy2DAsync(h_dst, d_pitch_dst, d_dst, d_pitch_dst, dst_width_bytes, dst_height,
                                 cudaMemcpyDeviceToHost, stream));
      try_cuda(cudaStreamSynchronize(stream));
      env->BitBlt(s_dst, s_pitch_dst, reinterpret_cast<const BYTE *>(h_dst), d_pitch_dst, dst_width_bytes, dst_height);
    }
...
public:
  Instance(std::string_view filterName, AVSValue args, IScriptEnvironment *env) : semaphore(num_streams) {
    auto items = this->items();
    new (items) Item(std::piecewise_construct, std::forward_as_tuple(filterName, args, env), std::forward_as_tuple());
    items[0].second.clear();
    for (unsigned i = 1; i < num_streams; ++i) {
      new (items + i) Item(std::piecewise_construct, std::forward_as_tuple(firstReactor()), std::forward_as_tuple());
...
PVideoFrame __stdcall EEDI2Filter::GetFrame(int n, IScriptEnvironment *env) {
  ...
  if (vi.BitsPerComponent() == 8) {
    auto &d = static_cast<Instance<uint8_t> *>(data)->acquireReactor();
    try {
      out = d.getFrame(n, env);
    } catch (const std::exception &exc) {
      env->ThrowError(("EEDI2CUDA: "s + exc.what()).c_str());
    }
    static_cast<Instance<uint8_t> *>(data)->releaseReactor(d);
  } else {
    auto &d = static_cast<Instance<uint16_t> *>(data)->acquireReactor();
    try {
      out = d.getFrame(n, env);
    } catch (const std::exception &exc) {
      env->ThrowError(("EEDI2CUDA: "s + exc.what()).c_str());
    }
    static_cast<Instance<uint16_t> *>(data)->releaseReactor(d);
  }
  return out;
}

EEDI2Filter::EEDI2Filter(std::string_view filterName, AVSValue args, IScriptEnvironment *env)
  ...
  try {
    ...
    if (vi.BitsPerComponent() == 8) {
      data = static_cast<void *>(new (num_streams) Instance<uint8_t>(filterName, args, env));
    } else {
      data = static_cast<void *>(new (num_streams) Instance<uint16_t>(filterName, args, env));
    }
  } catch (const std::exception &exc) {
    env->ThrowError(("EEDI2CUDA: "s + exc.what()).c_str());
  }
  ...
  if (vi.BitsPerComponent() == 8) {
    vi.width = static_cast<Instance<uint8_t> *>(data)->firstReactor().getOutputVI().width;
    vi.height = static_cast<Instance<uint8_t> *>(data)->firstReactor().getOutputVI().height;
  } else {
    vi.width = static_cast<Instance<uint16_t> *>(data)->firstReactor().getOutputVI().width;
    vi.height = static_cast<Instance<uint16_t> *>(data)->firstReactor().getOutputVI().height;
  }
}
...
#define eedi2_common_params                                                                                            \
  ...
  // env->AddFunction("EEDI2_CUDA_BuildConfig", "", EEDI2Filter::Create, nullptr);
